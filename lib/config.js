 ...
 * ignored). Style follows an earlier internal plugin's lib/config.js.
 *
...
const providerDefSchema = z.object({
  type: z.enum(['claude', 'codex', 'grok', 'acp']).optional(),
...
export const pluginConfigSchema = z.object({
  ...
  // D2b relay 回合闭环校验开关（DESIGN §5.4）：true（默认）挂
  // registerContinuableSetup guard，拒绝「本 epoch 零 subagent_submit 的
  // report」；false 完全关闭。presetRow 行分支不含此键（红线 9：官方行
  // 超集只扩全量分支；presetRow 实例无 bridge 状态）。
  relayReportGuard: z.boolean().optional(),
  // 输出脱敏开关（自 task-weaver redact.ts 移植，lib/redact.js）：true（默认）
  // 对 run.js 捕获的 stdout/stderr 与各 bridge 返回给模型的最终文本套用
  // 5 种秘密形态脱敏（Bearer / sk- / gh?_ / api_key= / JWT）——泄漏进对话
  // 上下文是真实危害，默认开启；false 恢复字节精确透传。presetRow 行分支
  // 不含此键（红线 9：presetRow 实例 native-only、无进程捕获面）。
  redactSecrets: z.boolean().optional(),
  // 引擎级 dispatch 缝（lib/dispatch.js，docs/dispatch-seam.md §3.2）：程序化
  // bridge 派发的 permissionMode 部署上限。缺省 'full'（与工具层 root 调用者
  // 对齐 —— seam 严格弱于模型工具面只会逼出「模型能干、代码不能干」的绕行，
  // 真正的边界在 parent-based 委派天花板）；越界请求 loud 拒绝、绝不静默降级。
  // presetRow 行分支不含此键（红线 9：该分支无 bridge 状态、不 provide 缝）。
  maxDispatchPermissionMode: z.enum(['readonly', 'default', 'full']).optional(),
  // —— 迁移 ——
