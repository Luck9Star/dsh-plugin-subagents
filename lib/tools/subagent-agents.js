...
//
// 自前身 legacy-bridges-plugin `lib/tools/product-agents.js` 迁移 + native 视图
...
export function registerSubagentAgents(ctx, deps) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: 'Overview of every delegation backend and live subagent: detected bridge CLI agents (codex / claude-code / configured ACP providers) with availability, the native subagent providers (spawn / fork), and every live subagent with its backend and activity.',
    parameters: {},
    output: {
    ...
    async execute(args, exec) {
      ...
      return { availability: availabilityView, native: nativeView, children }
    },
  }))
}
