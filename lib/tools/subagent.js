...
export function registerSubagentTool(ctx, deps) {
  ...
  const continuableDefault = config.backgroundMode === 'continuable'
  ...
  const description = wording.description
    + ' The optional `backend` parameter delegates to an external agent CLI (codex / claude-code / any configured ACP provider) instead, and `role` (subagent_roles lists them) bundles a backend default, the remote permission mode, and extra instructions.'
    + (backgroundEnabled
      ? (continuableDefault
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      ...
      backend: {
        type: 'string',
        enum: backendIds,
        description: `Backend for this delegation: "native" (default) runs a native in-process subagent; a bridge name (${backendIds.slice(1).join(' / ') || 'none detected on this deployment'}) delegates to that external agent CLI. Must agree with the chosen role's pinned backend when the role pins one.`,
      },
      role: {
        type: 'string',
    ...
    async execute(args, exec) {
      ...
      if (driver.kind === 'bridge') {
        ...
        const permissionMode = args.permission_mode !== undefined
          ? args.permission_mode
          : (role.permissionMode ?? 'default')
        assertWithinCeiling({
