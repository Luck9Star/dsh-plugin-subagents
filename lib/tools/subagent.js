...
export function registerSubagentTool(ctx, deps) {
  ...
  const backendBridgeSentence = bridgeNames.length > 0
    ...
    : (config.presetRow === true
      ? 'this tool is native-only; external agent CLIs (codex / claude-code / configured ACP providers) are delegated via the global "subagent" tool\'s backend parameter'
