...
export function registerSubagentSubmit(ctx, deps) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (!agent || !agent.session) throw new Error('subagent_submit requires a calling agent session')
      ...
      try {
        ...
        if (!record) {
          ...
          if (!persisted || !bridges[persisted.backend]) {
            throw new Error('subagent_submit: no remote product session is bound to this agent')
          }
