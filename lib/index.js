...
//
// 返回值：宿主忽略 apply 的返回；这里返回装配产物 { assembled, migration,
// presetRow } 供测试与内省使用。第三参 injections 是**测试专用**注入缝
 ...
 * @returns {Promise<{assembled: Object, migration: Object|null, presetRow: boolean}>}
 *          the host ignores this; exposed for tests/introspection
 */
export async function apply(ctx, config = {}, injections = {}) {
  ...
  if (cfg.presetRow === true) {
    ...
    return { assembled, migration: null, presetRow: true }
  }
  ...
  if (reg.wait) registerSubagentWait(ctx, { foldProgress, foldTrace })
  ...
  return { assembled, migration, presetRow: false }
}
