...
export const authChecks = {
  ...
  grok: () => {
    // File-artifact probe only (never executes the CLI — red line). grok
    // 1.0.4 stores credentials under ~/.grok/auth.json (verified on this
    // machine); absence does not block registration, it only surfaces the
    // hint, matching the claude-code behavior above.
    const path = join(home(), '.grok', 'auth.json')
    return existsSync(path)
      ? { ok: true, note: 'auth.json present (validity verified at call time)' }
      : { ok: false, note: '~/.grok/auth.json missing — run "grok login"' }
  },
