...
export function createGrokBridge(options = {}) {
  ...
  // grok 1.0.5's headless input preprocessing is heavy: a trivial single-turn
  // probe already reports ~37k input tokens (the CLI injects its full
  // skills/MCP inventory and system prompt before the task text). A
  // relay-scale task (multi-KB spec) needs far more wall time than the
  // 5-minute cross-bridge default to even REACH the model, so the grok
  // default is 15 minutes unless a config timeoutMs overrides.
  const timeoutMs = Math.max(1000, Number(options.timeoutMs) || 900000)
  ...
  const bridge = {
    ...
    async submit(remote, task, signal, cwd, settings = {}) {
      ...
      for (const line of stdout.split('\n')) {
        ...
        // `text` = the agent's turn message. grok 1.0.4 emitted ONE full
        // message event per turn; 1.0.5 streams the SAME logical message as
        // multiple token-slice `text` events ("PROBE", "-", "XY", …).
        // CONCATENATION covers both: a 1.0.4 single event appends whole
        // (assign-would semantics), a 1.0.5 run of slices reassembles the
        // message. Verified against a live 1.0.5 probe (streaming-json
        // single-turn): text deltas are additive, `thought` deltas are the
        // same shape and stay ignored.
      ...
      // grok 1.0.5 lockup recovery: `-s/--session-id` REFUSES an id that
      // already exists ("Error: Session ID ... is already in use") and does
      // NOT resume (CLI help: "errors if ... already in use under the target
      // session directory; does not resume"). A first turn can time out
      // AFTER the CLI persisted the conversation directory (1.0.5 persists
      // incrementally, before the terminal `end` event our NIT-3 gate keys
      // on), leaving remote.sessionId unset while the preallocated id IS
      // taken — every retry then re-sends the same `-s` and hard-fails
      // forever. The unlocked move (verified against a live locked session:
      // `--resume <id>` on the "in use" id answers normally, exit 0) is to
      // demote the preallocated id to a RESUME: the conversation exists on
      // the CLI, so continuing it is both legal and lossless.
      if (
        code !== null
        && code !== 0
        && !text
        && remote.sessionId === undefined
        && remote.pendingSessionId !== undefined
        && /already in use/i.test(stderr || stdout)
      ) {
        remote.sessionId = remote.pendingSessionId
        remote.pendingSessionId = undefined
        // Recurse ONCE with the resume path; the argv builder now emits
        // `--resume <id>` instead of the colliding `-s`.
        return bridge.submit(remote, task, signal, cwd, settings)
      }
