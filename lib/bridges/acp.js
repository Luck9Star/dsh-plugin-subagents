...
/**
 * Same 8MB tail bound as lib/run.js's appendCapped (R-1): the ACP transport
 * speaks stdio DIRECTLY (spawnProduct, not runCommand), so run.js's capture
 * cap does not apply here — an agent that streams without end must not be
 * able to grow textBuffer without bound. Only the TAIL is kept: the final
 * answer is read from the end (drainText), and progress.preview already
 * slices the last 200 chars.
 */
const MAX_TEXT_BUFFER_CHARS = 8 * 1024 * 1024

function appendCapped(current, chunk) {
  const next = current + chunk
  return next.length > MAX_TEXT_BUFFER_CHARS ? next.slice(next.length - MAX_TEXT_BUFFER_CHARS) : next
}
...
export function createAcpBridge(options = {}) {
  ...
  // redactSecrets threads config.redactSecrets (default true, lib/redact.js).
  // The acp transport speaks stdio DIRECTLY (spawnProduct, not runCommand),
  // so its capture is NOT covered by run.js's scrub — chunks are redacted as
  // they enter textBuffer/progress here, at the same "before any buffering"
  // boundary task-weaver uses.
    ...
    return {
      ...
      async readTextFile(params) {
        ...
        if (!root || !target || (target !== root && !target.startsWith(root + sep))) {
          throw new Error(`subagents: ACP readTextFile refused a path outside the workspace: ${params.path}`)
        }
      ...
      async writeTextFile() {
        throw new Error('subagents: ACP writeTextFile is not supported')
      },
  ...
  async function connect(cwd, signal) {
    ...
    try {
      ...
      const client = makeClient((text) => {
        // Redact at the buffering boundary: the conversation text and the
        // progress preview must never carry a secret the agent printed.
        // appendCapped keeps the buffer at the last 8MB (R-1: the direct
        // stdio transport has no run.js cap of its own) — receivedChars still
        // counts EVERYTHING, so progress shows the true stream size.
        const clean = scrub(text)
        textBuffer = appendCapped(textBuffer, clean)
        progress.lastChunkAt = Date.now()
        progress.receivedChars += clean.length
