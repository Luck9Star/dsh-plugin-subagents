...
export function createCodexBridge(options = {}) {
  ...
  // runCommand scrubs the capture; the final-text scrub below is idempotent.
  ...
  return {
    ...
    async submit(remote, task, signal, cwd, settings = {}) {
      ...
      if (remote.threadId) {
        // NIT-4: remote.threadId is captured from the CLI's untrusted JSONL
        // stream (`thread.started`), and `resume <thread_id>` is a SEPARATE
        // argv element the CLI would parse as a sub-command flag — a poisoned
        // stream giving a `-…`-shaped id could flip into another flag. Same
        // whitelist the `-c model=…` path uses: identifiers only, refuse
        // loudly (fail closed) on injection shapes.
        args.push('resume', safeConfigValue(remote.threadId, 'threadId'))
      }
      ...
      if (code !== 0) return { text: scrub(text), stopReason: 'error' }
      if (!text) {
        const plain = stdout.split('\n').filter((l) => l.trim() && !l.trim().startsWith('{')).join('\n').trim()
        if (plain) text = plain
      }
      return { text: scrub(text), stopReason: 'completed' }
    },
