...
export function createClaudeBridge(options = {}) {
  ...
  // belt-and-braces (a redacted placeholder never matches a pattern again).
  ...
  async function invoke(remote, args, signal, cwd) {
    ...
    const text = scrub(String(parsed.result ?? parsed.text ?? '').trim())
  ...
  function buildArgs(remote, task, settings) {
    ...
    if (remote.sessionId) args.push('--resume', safeFlagValue(remote.sessionId, 'sessionId'))
    else if (remote.pendingSessionId) args.push('--session-id', safeFlagValue(remote.pendingSessionId, 'sessionId'))
