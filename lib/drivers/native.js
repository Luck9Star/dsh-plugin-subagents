...
// `../native-delegate.js`（自旧版 cwd 插件抽出，逐行照搬）；本模块只做三路由
...
//
// cwd 补丁 stamp 门控（§6.4.2 定稿语义 + E-1/C-1 liveRoot 一致性）：
...
//     **且** stamp.liveRoot 与当前解析出的 live harness 根一致（见
//     resolveLiveHarnessRoot —— stamp 只对它点名的那个根有证明力；外来
//     stamp 曾随 tarball 泄漏、npx cache 漂移后旧 stamp 指向已废弃根，
//     两者都不得放行，否则未打补丁的 harness 会静默丢弃 cwd 字段）；
//   - 仅 `native`（版本白名单命中但未经 install 行为探针证实）**不产生任何
//     信任态** → 仍 throw 指引（「宁可误报漂移，不可误报 native」）；
//   - stamp 缺失 / 不可解析 / 字段不全 / liveRoot 不一致或不可解析 →
//     一律 throw 指引跑 `patches/install`。
...
/**
 * JS 侧的 live harness 根解析（resolve-root.sh 的镜像，E-1/C-1 驱动侧）。
 *
 * 原理与脚本侧相同：从「本插件实际链入的 harness peer 包」的真实物理位
 * 置向上走到第一个名为 node_modules 的目录，其父目录即 live root ——
 * `require.resolve` 会穿透 symlink（解析结果为真实路径），所以无论
 * peer 是 symlink（install.sh Stage A / setup:peer 修复后的形态，本机
 * 即此形态）还是真实目录，都能拿到包的最终落点：
 *
 *   <liveRoot>/node_modules/@deepseek-ai/dsh-tools  ← realpath 后的落点
 *
 * 选 dsh-tools 而非 dsh-subagent-in-process-driver：dsh-tools 是本插件
 * peerDependencies 的成员且必须与 harness 单实例共享（Symbol 去重，DESIGN
 * §6.4.4 / link-harness-dsh-tools.sh），是「本插件进程与 harness 是否同根」
 * 最强的既存证据；dsh-subagent-in-process-driver 并非本插件依赖，独立安装
 * 的插件目录下解析不到它。
 *
 * 解析失败（peer 未安装 / 目录形态意外）→ undefined：调用方把「无法解析
 * live root」与「stamp 缺失」同等对待 —— loud 失败，绝不静默放行。
 *
 * @returns {string|undefined} live harness 根的规范路径；解析不出则 undefined。
 */
function resolveLiveHarnessRoot() {
  try {
    const require = createRequire(import.meta.url)
    const pkgJsonPath = require.resolve('@deepseek-ai/dsh-tools/package.json')
    let dir = dirname(realpathSync(pkgJsonPath))
...
/**
 * 校验 cwd 补丁 stamp 处于可信状态（§6.4.2 定稿 + E-1/C-1 liveRoot 一致性）：
 *   1. 两枚补丁状态均为 `applied` 或 `native-verified`（`native` 独态不放行）；
 *   2. stamp.liveRoot 必须等于当前解析出的 live harness 根 —— stamp 记录的
 *      是「当时打补丁的那个根」，根已切换（npx cache 漂移）或 stamp 属于
 *      他人（曾随 tarball 泄漏的旧形态）时，对当前根断无证明力，放行即
 *      「未打补丁的 harness 静默丢弃 cwd」。
 *
 * `currentLiveRoot` 是「当前解析出的 live harness 根」；`undefined` 表示
 * 解析失败（与确认不一致同等对待：loud 失败，绝不静默放行）。由调用方
 * （驱动）计算并传入 —— 生产路径来自 resolveLiveHarnessRoot 的实例级
 * memo，测试路径用注入值替换。
 *
 * @param {string} stampPath stamp 文件路径
 * @param {string|undefined} currentLiveRoot 当前 live harness 根（undefined = 无法解析）
 * @returns {Promise<void>}
 * @throws {Error} stamp 缺失 / 不可解析 / 字段不全 / 存在不可信状态 /
 *   liveRoot 与当前 harness 根不一致 / live root 无法解析。
 */
async function assertCwdPatchesTrusted(stampPath, currentLiveRoot) {
  let raw
  try {
  ...
  // E-1/C-1: the stamp only proves anything about the root it names. A stamp
  // from another machine (previously shipped in the tarball) or from a
  // previous npx cache root must never unlock cwd against the CURRENT root.
  const stampLiveRoot = typeof doc.liveRoot === 'string' && doc.liveRoot.length > 0 ? doc.liveRoot : undefined
  if (currentLiveRoot === undefined) {
    throw cwdPatchError(
      'cannot resolve the current live harness root '
      + '(expected the plugin to share @deepseek-ai/dsh-tools with the running harness)',
    )
  }
  if (stampLiveRoot === undefined) {
    throw cwdPatchError(`stamp file has no liveRoot record (expected "${currentLiveRoot}")`)
  }
  if (stampLiveRoot !== currentLiveRoot) {
    throw cwdPatchError(
      `stamp liveRoot mismatch: stamp says "${stampLiveRoot}" but the current harness root is "${currentLiveRoot}"`
      + ' (npx cache drift, a harness switch, or a foreign stamp — the cwd patches were NOT applied to this root)',
    )
  }
}
 ...
 * @param {string} [spec.liveRootOverride] 可选 live harness 根覆盖（测试注入；缺省按 dsh-tools 落点动态解析）
 * @param {boolean} [spec.liveRootUnresolvable] 测试注入：强制 liveRoot 解析失败（模拟 peer 缺失；不进生产配置面）
 * @returns {import('./types.js').SubagentDriver}
 */
export function createNativeDriver({ kind, ctx, config, presetsRoot, stampPath, liveRootOverride, liveRootUnresolvable }) {
  ...
  // Config-side delegation-depth default (F1): the official
  // `@deepseek-ai/dsh-tool-subagent` and its earlier internal predecessor both
  // default `maxDepth: 3`, and when it is numeric it is forwarded per-request.
  // `'provider-managed'` keeps its pass-through meaning — not forwarded; the
  // provider governs depth autonomously. Omitted → 3 (bounds the default).
  const configMaxDepth = config.maxDepth ?? 3
  const id = `native:${kind}`
  const stampFile = stampPath !== undefined ? stampPath : DEFAULT_STAMP_PATH
  // liveRoot resolution result, memoized per driver instance: the peer layout
  // cannot change within one process, and the check runs on every cwd call.
  // Only a SUCCESSFUL resolution is memoized — a failure (peer not yet linked,
  // e.g. before install.sh) is NOT cached, so the next check retries and picks
  // up the peer the moment it appears (installed without restarting the app).
  let memoLiveRoot
  const currentLiveRoot = () => {
    if (memoLiveRoot === undefined && !liveRootUnresolvable) {
      memoLiveRoot = resolveLiveHarnessRoot()
    }
    return liveRootUnresolvable ? undefined : memoLiveRoot
  }
  ...
  return {
    ...
    async start(request) {
      ...
      // Depth resolution priority: per-call number > per-call 'provider-managed'
      // (explicit opt-out, not forwarded) > config default (numeric forwarded,
      // 'provider-managed' not forwarded, omitted → 3).
      let maxDepth
      if (typeof native.maxDepth === 'number') {
        maxDepth = native.maxDepth
      } else if (native.maxDepth === 'provider-managed') {
        maxDepth = undefined
      } else {
        maxDepth = configMaxDepth === 'provider-managed' ? undefined : configMaxDepth
      }
      let cwd
      if (native.cwd !== undefined) {
        cwd = assertCwd(native.cwd)
        await assertCwdPatchesTrusted(stampFile, liveRootOverride !== undefined ? liveRootOverride : currentLiveRoot())
      }
