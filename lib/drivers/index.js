...
//     的 provider 建 driver（逻辑参照旧版桥接插件 lib/index.js
...
export async function assembleDrivers({ ctx, config = {} }) {
  ...
  // redactSecrets（config，默认 true）随 buildProviders 织入每个 provider def，
  // 再经 createBridgeFor 传入各 bridge（lib/redact.js 的开关线）。
  const providers = buildProviders({ providers: config.providers ?? {}, ...(config.redactSecrets === false ? { redactSecrets: false } : {}) })
