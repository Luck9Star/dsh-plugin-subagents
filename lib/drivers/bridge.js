...
export function createBridgeState({ registry, idleTimeoutMs, pendingStartGuardMs } = {}) {
  ...
  const markEnded = (childId) => {
    ...
    if (endedAt.size > 128) {
      ...
      for (const [id, at] of endedAt) if (at < cutoff) endedAt.delete(id)
    }
  }
 ...
 *  - end：markEnded + liveChildren.delete（放槽）+ binding 命中 →
 *    scheduleDispose（idle 释放）；
  ...
  return {
    ...
    async start(request) {
      ...
      if (request.route === 'sync') {
        ...
        const cwd = parentCwdFn(request.parent)
        const remote = await bridge.create(cwd)
        try {
