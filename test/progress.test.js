...
test('foldProgress folds subagent_submit tasks (new tool face)', () => {
  const now = Date.now()
  const session = { events: events([
    { type: 'turn/start', timestamp: now - 1000, payload: { turn: 1 } },
    { type: 'tool/call', timestamp: now - 900, payload: { name: 'subagent_submit', args: { task: 'explore the repo' } } },
  ]) }
  const out = foldProgress(session)
  assert.equal(out.turn, 1)
  assert.equal(out.lastTask, 'explore the repo')
})

test('foldProgress handles both submit names without misjudging', () => {
  const now = Date.now()
  // a legacy product_submit call followed by a fresh subagent_submit call in
  // the same window: the newest submit task wins and both fold as tasks.
  const session = { events: events([
    { type: 'tool/call', timestamp: now - 2000, payload: { name: 'product_submit', args: { task: 'old task' } } },
    { type: 'tool/call', timestamp: now - 1000, payload: { name: 'subagent_submit', args: { task: 'new task' } } },
  ]) }
  const out = foldProgress(session)
  assert.equal(out.lastTask, 'new task', 'newest submit wins')
})
...
test('foldTrace labels subagent_submit calls with the matched name', () => {
  const now = Date.now()
  const session = { events: events([
    { type: 'tool/call', timestamp: now, payload: { name: 'subagent_submit', args: { task: 't' } } },
  ]) }
  const trace = foldTrace(session, 10)
